    ...
    ventures: [
      {
        ...
        detail: 'Security gateway between AI agents and enterprise systems. Every call crosses an eight-stage fail-closed pipeline — parse, authenticate, rate-limit, apply policy, tokenise PII, route by jurisdiction, execute, audit — and a stage that cannot reach a verdict refuses the call rather than letting it through. Personal data never leaves the perimeter in clear: a Rust tokeniser recognises the MENA identity formats generic scanners miss. Each request is pinned to the jurisdiction that governs it, and the audit trail is an Ed25519-signed hash chain, so a tampered record breaks the chain instead of quietly rewriting history. Go and Rust, Docker-ready, built for regulated financial institutions across MENA and the EU.',
        capabilities: [
          'MCP reverse proxy for AI agents',
          'Eight-stage fail-closed pipeline',
          'Authentication & per-caller rate limits',
          'Policy verdict before execution',
          'Rust PII tokeniser',
          'MENA identity formats — IBAN, CIN, Iqama, Emirates ID',
          'Luhn card-number detection',
          'Jurisdiction-pinned routing — MA, KSA, UAE, EU',
          'Ed25519-signed hash-chain audit',
          'Tamper-evident, replayable trail',
          'CISO dashboard',
    ...
    ventures: [
      {
        ...
        detail: 'Wealth structuring for expat families whose assets sit across four or more jurisdictions. The architecture is Zero Access: keys stay with the client, so the platform holds what it cannot read — and cannot be compelled to surrender what it has no way to decrypt. Above that sits a consolidated view of portfolios, liabilities and estate documents in every currency the family actually holds, and a gateway that shows an advisor, a spouse or an heir exactly the slice they are entitled to and nothing past it. Succession covers Islamic Faraid shares alongside civil-law regimes, and a Dead Man Switch releases what was meant to be released once it can no longer be asked for. €1,500–€4,000 a year.',
        capabilities: [
          'Zero Access encryption — client-held keys',
          ...
          'Islamic / Faraid succession shares',
          'Civil-law succession regimes',
          'Dead Man Switch release',
      ...
      },
      {
        ...
        detail: 'Treasury intelligence for the corporates that have outgrown a spreadsheet and will never buy Kyriba or ION. Designed on sixteen years of payments infrastructure across 83 countries, and ISO 20022 native rather than retrofitted — the richer remittance data survives the whole journey instead of being truncated into a legacy field on the way. Cash visibility across banks and currencies, FX exposure, and payment orchestration that reuses KM OpenPay for execution rails. The modules below are specified and designed; the badge says prototype for a reason.',
        capabilities: [
    ...
    ventures: [
      {
        ...
        detail: 'One identity, several faces. A single profile carries separate views — Social, Work, Exclusive — and each is a different person to a different audience: its own content, its own QR code, its own Apple Wallet pass. Someone who scans the pass at a work event reaches the work face; the social one stays where it belongs. Money moves on the same surface rather than on a third-party page: tips, a shop and donations run through Stripe. An email signature generator carrying the Oo icon turns every message the holder sends into a distribution channel.',
        capabilities: [
          'Per-view identity — Social, Work, Exclusive',
          'A QR code per view',
          'Apple Wallet pass per view',
          'Vanity profile URLs',
          'Tipping through Stripe',
          'Integrated shop',
          'Donations',
    ...
    ventures: [
      {
        ...
        capabilities: [
          ...
          'Souffleur IA — plain-language assistant',
